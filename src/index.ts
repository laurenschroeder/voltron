...
// Delay IWSDK init until video splash finishes (WebGL context kills video playback)
const videoOverlay = document.getElementById("video-splash");
if ((videoOverlay as any)?._done) {
  bootApp();
} else {
  (window as any)._onVideoSplashDone = bootApp;
}
